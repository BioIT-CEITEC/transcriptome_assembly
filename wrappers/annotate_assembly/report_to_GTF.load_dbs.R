...
map_to_uniprot_name = function(id, srcs) {
  ...
  if(res[,.N]>0) {
    if(res[,.N]==1) {
      ...
      # here, the uniprot IDs must be stored in tmp file for grep -f
      gfile = tempfile() #"test_res" #temp
      fwrite(as.list(res$uniprot_acc), gfile, sep = '\n', col.names = F, row.names = F)
      names = suppressWarnings(fread(cmd=paste0("zcat ",path_to_uniprot_mapping,"Gene_Name.gz | grep -wFf ",gfile), header = F, col.names = c("uniprot_acc","uniprot_name"), key="uniprot_acc"))
      if(names[,.N]>0) {
        res = names[res, on="uniprot_acc"]
      } else {
        # not matching UniProt name found for UniProt accession
    ...
    if(res[!is.na(uniprot_name), .N]>0) {
      return(res[,.N,by=uniprot_name][which.max(N),uniprot_name])
    }
  }
  return("")
}
...
process_blast_evidence = function(dt, col) {
  ...
  best_ev = dt[get(col) != '.', as.data.table(stri_split_fixed(unlist(stri_split_fixed(get(col),'`')),'^', simplify = T))[,V5:=as.numeric(sub('^E:','',V5))][which.min(V5), c(V1,source,paste0(V1,',',V3,',',V4,',E:',V5))]]
  best_ev[1] = ifelse("MEROPS" %in% dbs, merops[id == best_ev[1], acc], best_ev[1])
  if(length(dbs)>0) {
...
best_evidence = function(dt) {
  ...
  if(sum(dt$sprot_Top_BLASTX_hit != '.')>0) {
    ...
    best_ev = dt[sprot_Top_BLASTX_hit != '.', as.data.table(stri_split_fixed(unlist(stri_split_fixed(sprot_Top_BLASTX_hit,'`')),'^', simplify = T))[,V5:=as.numeric(sub('^E:','',V5))][which.min(V5), c(V1,"UniProtKB/Swiss-Prot",paste0(V1,',',V3,',',V4,',E:',V5))]]
  ...
  if(sum(dt$sprot_Top_BLASTP_hit != '.')>0) {
    ...
    best_ev = dt[sprot_Top_BLASTP_hit != '.', as.data.table(stri_split_fixed(unlist(stri_split_fixed(sprot_Top_BLASTP_hit,'`')),'^', simplify = T))[,V5:=as.numeric(sub('^E:','',V5))][which.min(V5), c(V1,"UniProtKB/Swiss-Prot",paste0(V1,',',V3,',',V4,',E:',V5))]]
    uname = map_to_uniprot_name(best_ev[1], "UniProtKB-ID")
    if(uname != '') {
      usrc = "UniProt"
    } else {
...
args = c("stage519_Abramis_rutilus_TAssembly.Rutilus_rutilus_plotice/denovo_transcriptome_assembly/assembly/rutilus/annotation/trinotate_report_wt_seqs.xls",
         "stage519_Abramis_rutilus_TAssembly.Rutilus_rutilus_plotice/denovo_transcriptome_assembly/assembly/rutilus/annotation/transdecoder_dir/rutilus.merged.fa.transdecoder.pep",
         "stage519_Abramis_rutilus_TAssembly.Rutilus_rutilus_plotice/denovo_transcriptome_assembly/assembly/rutilus/annotation/rutilus.trinotate_annotation.uniform_ids.gtf",
         "refseq,merops",
