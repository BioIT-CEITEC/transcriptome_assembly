###########################################
# wrapper for rule: prepare_annot_blastx
###########################################
import os
import sys
import math
...
command = "$(which time) diamond blastx"+\
...
command = '$(which time) Rscript -e  \'data.table::fwrite(data.table::fread("'+snakemake.params.blast_full+'")'+\
          ...
          ' & (V9*3-V14)/V2>='+str(snakemake.params.cov_cutof)+',.SD[order(-V10,V11,V14)][1],by=V1][,.(V1,V5,V12,V9,V14,V15,V3,V4,V7,V8,V11,V10)]'+\
