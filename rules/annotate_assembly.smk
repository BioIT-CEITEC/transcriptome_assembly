...
def annotate_assembly_inputs(wc):
    ...
    inputs['sqlite'] = GLOBAL_TMPD_PATH+"Trinotate.sqlite"
    inputs['custom'] = expand("results/annotation/blast{custom}.tsv", custom=["x_"+i+"_processed" for i in custom_prot_dbs]+["p_"+i+"_processed" for i in custom_prot_dbs]+["n_nt_processed.taxids_"+i.split(':')[0] for i in nt_taxids])
    # necessary Uniprot-mapping DBS: EMBL, MEROPS, RefSeq, RefSeq_NT
    return inputs
...
rule annotate_assembly:
    ...
    params: prefix = "results/annotation",
            ...
            custom_nt_taxid= [i.split(':')[0] for i in nt_taxids],
...
rule prepare_annot_signalp:
    input:  pep = "results/annotation/transdecoder_dir/longest_orfs.pep",
            ...
            # licence = GLOBAL_REF_PATH+"general/SignalP/signalp-4.1g.Linux.tar.gz",
            licence = GLOBAL_REF_PATH+"general/SignalP/signalp-5.0b.Linux.tar.gz",
...
rule prepare_annot_pfamdb:
    input:  fa = "results/annotation/transdecoder_dir/longest_orfs.pep",
            pfam_db = GLOBAL_TMPD_PATH+"Pfam-A.hmm",
...
rule prepare_annot_blastn:
    input:  fa = "results/assembly/merged/merged_samples.merged.fa",
            db = GLOBAL_REF_PATH+"general/nucleotide_DB/nt_db.ndb"
    ...
    params: blast_full= "results/annotation/blastn_{db}_full.taxids_{tax}.tsv",
            ...
            cov_cutof = 0.8,
            taxid = lambda wcs: [i.split(':')[1] for i in nt_taxids if i.split(':')[0] == wcs.tax][0],
...
def prepare_db_for_blast_rules(wcs):
    if "merops" in wcs.db:
        return GLOBAL_REF_PATH+"general/MEROPS_DB/merops_pepunit.diamond_db.dmnd"
    if "refseq" in wcs.db:
        return GLOBAL_REF_PATH+"general/RefSeq_DB/NCBI_RefSeq_complete_nonredundant_protein.diamond_db.dmnd"
    else:
        return GLOBAL_REF_PATH+"general/UniProt_SwissProt_DB/uniprot_sprot.dmnd"
...
#     if "merops" in wcs.db:
#         return GLOBAL_REF_PATH+"/general/MEROPS_DB/merops_pepunit.fa.gz"
#     if "refseq" in wcs.db:
#         return GLOBAL_REF_PATH+"/general/RefSeq_DB/NCBI_RefSeq_complete_nonredundant_protein.fa.gz"
#     else:
#         return GLOBAL_REF_PATH+"/general/UniProt_SwissProt_DB/uniprot_sprot.fa.gz"
# 
# rule prepare_annot_makeblastdb(wcs):
#     input:  fa = prepare_annot_makeblastdb_input,
#     output: db_ok = GLOBAL_TMPD_PATH+"blastdb_{db}.ok",
#     log:    run = "logs/prepare_annot_makeblastdb_for_{db}.log"
#     threads: 10
#     resources:  mem = 30
#     params: simple_names = GLOBAL_TMPD_PATH+"uniprot_sprot.simple_names.fasta",
#             working_dir = "./",
...
rule prepare_annot_build:
    output: pfam_db = GLOBAL_TMPD_PATH+"Pfam-A.hmm",
            sqlite = GLOBAL_TMPD_PATH+"Trinotate.sqlite",
    log:    run = "logs/prepare_annot_build.log"
    threads: 1
    resources:  mem = 5
    params: prefix = GLOBAL_TMPD_PATH,
            pfam_db = GLOBAL_TMPD_PATH+"Pfam-A.hmm.gz",
